...
class SudokuGrid extends Grid<SudokuCell> {
  ...
  /// Asynchronously generates a game using a background isolate.
  static Future<SudokuGrid> generateGameAsync({
    required int gridSize,
    String difficulty = 'medium',
    GenerationMode mode = GenerationMode.symmetric,
    int? seed,
    ...
    final completer = Completer<SudokuGrid>();
  ...
  /// Retrieves the number of conflicts currently hitting the cell at [row] x [col].
  int getConflictCount(int row, int col) => _conflictCounts[row * gridLength + col];
