...
final class SudokuGenerator {
  ...
  static void generate(
    ...
    SudokuGenerator._generateGrid(grid, diff.cellsToRemove(), mode, rng, diff.currentDifficulty.logicLevel);
  }

  /// Private helper responsible for generating the grid.
  static void _generateGrid(SudokuGrid grid, int cellsToRemove, GenerationMode mode, Random rng, int targetLevel) {
    // Fill the grid with the necessary data
    SudokuGenerator._fillGrid(grid, 0, 0, rng);

    // Remove cells from the grid.
    SudokuGenerator._removeCells(grid, cellsToRemove, mode, rng, targetLevel);
  ...
  static void _removeCells(SudokuGrid grid, int cellsToRemove, GenerationMode mode, Random rng, int targetLevel) {
    // Remove cells based on the mode
    switch (mode) {
      case GenerationMode.symmetric:
        SudokuGenerator._removeSymmetrically(grid, cellsToRemove, rng, targetLevel);
      case GenerationMode.random:
        SudokuGenerator._removeRandomly(grid, cellsToRemove, rng, targetLevel);
    }
  }

  /// Randomly removes cells from the grid.
  static void _removeRandomly(SudokuGrid grid, int cellsToRemove, Random rng, [int targetLevel = 1]) {
  ...
  static void _removeSymmetrically(SudokuGrid grid, int cellsToRemove, Random rng, [int targetLevel = 1]) {
